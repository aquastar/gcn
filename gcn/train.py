...
flags.DEFINE_integer('eig_dim', 50, 'Maximum eigen value number.')
...
if True:
    rat_model = RAT_after_GCN(placeholders, input_dim=features[2][1], gcn=model, support_inv=support_inv, logging=True)
    model = rat_model
    sess.run(tf.global_variables_initializer())
